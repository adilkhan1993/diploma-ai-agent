...
async def agent_streamer(history_messages: list[Message]):
    try:
        ...
        # 1. Строгий системный промпт (защита от галлюцинаций)
        messages = [
            {
                "role": "system", 
                "content": (
                    "Ты — профессиональный ИИ-аналитик. "
                    "ВНИМАНИЕ: Используй инструмент 'scrape_website' ТОЛЬКО если пользователь ЯВНО написал ссылку, начинающуюся с http:// или https://. "
                    "НИКОГДА не придумывай и не генерируй ссылки самостоятельно! "
                    "Если пользователь задает обычный вопрос (без ссылки) или прислал контекст документа, просто отвечай из своей базы знаний, НЕ ВЫЗЫВАЯ инструменты."
                )
            }
        ]
